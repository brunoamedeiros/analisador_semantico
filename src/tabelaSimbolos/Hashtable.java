...
public class Hashtable {
	...
	public Element getObj(String name) {
		if (name == null)
			return null;

		int index = Hash.hash(name, this.ARR_SIZE);
		LinkedList<Element> items = this.arr[index];
	...
	public Element getObj(String name, int level) {
		if (name == null)
			return null;
		...
		for (Element item : items) {
			if (item.getName().equals(name) && item.getNivel() == level)
				return item;
		}
	...
	public Element get(String name) {
		Element item = getObj(name);

		return item;
	}

	public Element get(String name, int level) {
		Element item = getObj(name, level);
	...
	public boolean objExists(Element element) {
		if (getObj(element) != null) {
			...
			System.out.println("Nome: " + element.getName() + " Categoria: " + element.getCategoria() + " Nível: "
					+ element.getNivel() + " Geral A: " + element.getAllA() + " Geral B: " + element.getAllB() + "\n");
			return true;
		}
	...
	public boolean objExists(String name, int level) {
		Element element = get(name, level);
		...
		for (LinkedList<Element> item : this.arr) {
			if (item != null) {
				for (Element element : item) {
					System.out.format(leftAlignFormat, element.getName(), element.getCategoria(), element.getNivel(),
							element.getAllA(), element.getAllB());
				}
			}
		}
		System.out.format("+-----------+------------+-------+---------+---------+ %n\n");
